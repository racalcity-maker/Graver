...
class VectorJobExecutor {
 public:
  VectorJobExecutor(motion::MotionService &motion, laser::LaserService &laser);
  ...
  esp_err_t execute(const VectorTextJob &job, std::atomic_bool &stopRequested, std::atomic_bool &pauseRequested,
                    std::atomic_uint32_t &unitsDone, std::atomic_uint32_t &unitsTotal,
                    std::atomic_uint8_t &progressPercent);
