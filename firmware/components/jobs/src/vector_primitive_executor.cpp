...
VectorPrimitiveExecutor::VectorPrimitiveExecutor(motion::MotionService &motion, laser::LaserService &laser)
    : motion_(motion), laser_(laser) {}

esp_err_t VectorPrimitiveExecutor::execute(const VectorPrimitiveJob &job, std::atomic_bool &stopRequested,
  ...
  std::vector<std::vector<PointMm>> contours;
  contours.reserve(contour_count);
  ...
  for (uint32_t contour_index = 0; contour_index < contour_count; ++contour_index) {
    ...
    std::vector<PointMm> contour = makeContour(job.manifest, inset);
    if (contour.size() < 2U) {
      break;
    }
    total_segments += static_cast<uint32_t>(contour.size());
  ...
  ESP_LOGI(kTag, "Run vector primitive id=%s shape=%s size=%.3fx%.3f stroke=%.3f contours=%u power=%u print=%.1f travel=%.1f",
           ...
           static_cast<unsigned>(contours.size()), static_cast<unsigned>(job.manifest.power),
  ...
  err = moveAndWait(motion_, 0.0f, 0.0f, job.manifest.travelSpeedMmMin, stopRequested);
  if (err != ESP_OK) {
    cleanup_laser();
    return err;
  }
  ...
  for (const std::vector<PointMm> &contour : contours) {
    if (stopRequested.load()) {
      cleanup_laser();
      return ESP_ERR_INVALID_STATE;
    }
    ...
    err = moveAndWait(motion_, contour.front().x, contour.front().y, job.manifest.travelSpeedMmMin, stopRequested);
    if (err != ESP_OK) {
      cleanup_laser();
      return err;
    }

    err = waitWhilePaused(laser_, motion_, stopRequested, pauseRequested);
    if (err != ESP_OK) {
      cleanup_laser();
      return err;
    }

    err = laser_.setPower(job.manifest.power);
    if (err != ESP_OK) {
      cleanup_laser();
      return err;
    }

    for (size_t point_index = 1; point_index <= contour.size(); ++point_index) {
      const PointMm &point = contour[point_index == contour.size() ? 0U : point_index];
      err = moveAndWait(motion_, point.x, point.y, job.manifest.printSpeedMmMin, stopRequested);
      if (err != ESP_OK) {
        cleanup_laser();
        return err;
      }
